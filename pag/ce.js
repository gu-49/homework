function publicHeader(target){ //target 事件属性可返回事件的目标节点（触发该事件的节点），如生成事件的元素、文档或窗口。
    ...
    if(childArr.length != 0){
     ...
     titleimg +=`
      ...
      for(var j=0;j<childArr.length;j++){
        ...
        var Positioning_arrid=childArr[j].label;
        childText +=`
 ...
 /*   var userText=``;
     ...
     if(childArr.length != 0){
      //沒有值的選單不要顯示圖片
       for(var j=0;j<childArr.length;j++){
         ...
         childClass=`nvbar-item-child`;
         childText +=`
         <div id="${item.id}.id" class="nvbar-item-child-title" onclick="handleNavbar(${item.id})">${childArr[j].label}</div>
         `; 
       }
   ...
   `;
   $("#"+target).html(text);  //拼接調用index.js     ${userText}
 }
...
/*function publicRoot(target) {
  text=`
    <div class="task_Positioning">
      <span class="iconfont icon-dingweiweizhi" style="display:inline-block;position:relatvie;"></span>
      <span style="color: dimgrey; font-size: 12px;">当前位置：首页 > </span>
    </div>
  `;
  $('#'+target).html(text);
}*/